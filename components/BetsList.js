...
function Column({ side, list, past }) {
  ...
  return (
    <div className={'vs-col ' + (side === 'Миша' ? 'pasha' : 'ai')}>
    ...
    <div>
      ...
      <div className="controls">
        ...
        <div className="seg sides" role="group" aria-label={T('a11y.side')}>
          {SIDES.map((k) => (
            <button key={k} className={'seg-btn' + (side === k ? ' on' : '') + (k === 'Миша' ? ' t-pasha' : k === 'AI' ? ' t-ai' : '')} aria-pressed={side === k} onClick={() => setSide(k)}>
              {sideBtnLabel(k)}
            </button>
          ))}
        </div>
...
const normName = (s) => s.replace(/\s*\((?:Миша|AI)\)\s*$/i, '').trim();
...
function ByMatch({ list, single }) {
  ...
  const sysSort = (a, b) => {
    ...
    return a.side === b.side ? 0 : a.side === 'Миша' ? -1 : 1;
  };
