...
const NEWS_TAG_CLS = { lineup: 'lineup', insight: 'insight', misha: 'pasha', result: 'result' };
...
function Avatar({ pasha, lang }) {
  if (pasha) return <span className="news-av pasha" aria-hidden="true">{lang === 'en' ? 'M' : 'М'}</span>;
  return (
    ...
    <aside className="news" aria-label={T('a11y.newsFeed')}>
      ...
      <ol className="news-list">
        {sorted.map((n, i) => {
          ...
          const isPasha = n.tag === 'misha';
          return (
            <li key={i} className="news-item">
              <Avatar pasha={isPasha} lang={lang} />
              <div className="news-main">
                <div className="news-meta">
                  <span className="news-author">{isPasha ? T('news.tag.misha') : T('news.author.staff')}</span>
