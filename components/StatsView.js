...
const PASHA = bets.filter((b) => b.side === 'Миша');
...
function BankChart() {
  ...
  for (const b of settled) {
    if (b.side === 'Миша') p += pl(b); else a += pl(b);
    P.push(p); A.push(a);
  }
  ...
  return (
    <div>
      ...
      <div className="chart-legend">
        <span><i style={{ background: '#d97706' }} />{sideLabel('Миша', lang)} <b className="num">{money(p, lang)}</b></span>
  ...
  return (
    <div className="duo">
      <div className="duo-col pasha">
        <div className="duo-hd"><span className="mc-av pasha" aria-hidden="true">{lang === 'en' ? 'P' : 'П'}</span>{sideLabel('Миша', lang)}</div>
        {render(PASHA)}
      </div>
