...
export default function BattleBoard() {
  const lang = useLang();
  const t = useT();
  const pasha = aggregate(bets.filter((b) => b.side === 'Миша'));
  const ai = aggregate(bets.filter((b) => b.side === 'AI'));
  const leaderSide = pasha.bal === ai.bal ? null : pasha.bal > ai.bal ? 'Миша' : 'AI';
  ...
  return (
    <section aria-label={t('a11y.battle')}>
      <div className="battle">
        <Side name={sideLabel('Миша', lang)} agg={pasha} cls="pasha" />
